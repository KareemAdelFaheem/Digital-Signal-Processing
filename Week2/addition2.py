...
def addition():
    ...
    signal3 = filedialog.askopenfilename(
        initialdir="Lab 2/input",
        title="Signal 2",
    )
    ...
    file_label3.config(text=f"Selected File: {signal3}")
    ...
    if signal3:
        with open(signal3, "r") as f:
            for i in range(3):
                next(f)

            for line in f:
                parts = line.strip().split()
                y3_value.append(float(parts[1]))

    if signal3:
        for i in range(0, len(y1_value)):
            newy_value.append(y1_value[i] + y2_value[i]+y3_value[i])
    else:
        for i in range(0, len(y1_value)):
...
file_label3 = tk.Label(root, text="Selected File: ", pady=40)
